...
public class APDynamicResources implements IResourcePack
{
	...
	{
		if (packType != ResourcePackType.CLIENT_RESOURCES) return invalidResource(resource, "invalid pack type " + packType);
		...
		return getResources(packType, domain, path)
				.filter(file -> {
					if (file.startsWith(startingFolder))
					{
						...
						if (relativePathSplit.length <= maxDepth && fileNameFilter.test(relativePathSplit[relativePathSplit.length - 1])) return true;
						else return false;
					}
					else return false;
				})
	...
	public Stream<String> getResources(ResourcePackType packType, String domain, String path)
	{
		if (packType == ResourcePackType.CLIENT_RESOURCES && AdditionalPlacementsMod.MOD_ID.equals(domain)) {
