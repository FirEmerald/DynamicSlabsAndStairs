...
public class BlockModelCache {
	...
	public static BakedModel bake(UnbakedModel model, ModelBaker baker, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelTransform) {
		return OUR_MODEL_CACHE.computeIfAbsent(new OurModelKey(model, modelTransform.getRotation(), modelTransform.isUvLocked()), key -> BlockModelCache.unwrap(model.bake(baker, spriteGetter, modelTransform)));	
	}
	...
	public static BakedModel bake(UnbakedModel model, ModelBaker baker, Function<Material, TextureAtlasSprite> spriteGetter) {
		return THEIR_MODEL_CACHE.computeIfAbsent(new TheirModelKey(model), key -> BlockModelCache.unwrap(model.bake(baker, spriteGetter, BlockModelRotation.X0_Y0)));
	}
