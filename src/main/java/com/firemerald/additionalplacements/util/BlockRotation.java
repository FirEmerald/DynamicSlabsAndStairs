...
public enum BlockRotation
{
	...
	public void rotateUV(int[] oldData, int oldPos, int[] newData, int newPos, int rotateUV, TextureAtlasSprite tex) {
		switch (rotateUV) {
		case 1:
			newData[newPos + 0] = Float.floatToRawIntBits(tex.getU(16 - BlockModelUtils.getVOffset(tex, Float.intBitsToFloat(oldData[oldPos + 1])))); //1-V
			newData[newPos + 1] = Float.floatToRawIntBits(tex.getV(     BlockModelUtils.getUOffset(tex, Float.intBitsToFloat(oldData[oldPos + 0])))); //U
			break;
		case 2:
			...
			break;
		case 3:
			newData[newPos + 0] = Float.floatToRawIntBits(tex.getU(     BlockModelUtils.getVOffset(tex, Float.intBitsToFloat(oldData[oldPos + 1])))); //V
			newData[newPos + 1] = Float.floatToRawIntBits(tex.getV(16 - BlockModelUtils.getUOffset(tex, Float.intBitsToFloat(oldData[oldPos + 0])))); //1-U
			break;
		}
	}
	...
	public VoxelShape applyBlockSpace(VoxelShape shape) {
		return shapeCache.computeIfAbsent(shape, nil -> {
	        ...
	        shape.forAllBoxes((minX, minY, minZ, maxX, maxY, maxZ) -> buffer[0] = VoxelShapes.or(buffer[0], createRotatedBox(minX, minY, minZ, maxX, maxY, maxZ)));
	        return buffer[0];
		});
	}
