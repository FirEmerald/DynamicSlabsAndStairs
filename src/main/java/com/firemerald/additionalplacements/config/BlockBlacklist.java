...
public class BlockBlacklist extends GenerationBlacklist {
	...
	public boolean test(ResourceLocation id, Block block) {
		...
		if (block.getTags().stream().anyMatch(tagKey -> tagBlacklist.contains(tagKey.toString()))) flag = false;
		if (block.getTags().stream().anyMatch(tagKey -> tagWhitelist.contains(tagKey.toString()))) flag = true;
